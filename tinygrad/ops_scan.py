...
def associative_scan(fn:Callable[[Tensor, Tensor], Tensor], elems:Tensor, axis:int=0, reverse:bool=False) -> Tensor:
  ...
  ``fn`` must be associative and operate elementwise on tensors with matching shapes.
  The implementation uses a divide-and-conquer upsweep/downsweep-style recursion with
  O(log N) dependency depth, similar to JAX ``lax.associative_scan``.
  ...
  Args:
    ...
    reverse: scan from right to left when true.
  """
